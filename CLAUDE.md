...
- `admin/` - Bootstrap 5 admin interface
  - `js/admin-template.js` - Navbar/sidebar generator with `initTemplate(section)` function
  - `js/api-admin.js` - API client with JWT handling
- `vendor/` - Local copies of external libraries (NO CDN usage)
  - `bootstrap/` - Bootstrap CSS and JS
  - `bootstrap-icons/` - Bootstrap Icons font
  - `sweetalert2/` - SweetAlert2 (modals, toasts, confirmations)
...
All admin pages follow a consistent template structure:
    ...
    <div class="row">
      ...
      <div class="col-md-10">
        <div class="d-flex justify-content-between align-items-center mb-4">
          <div>
            <h1 class="h3 mb-0"><i class="bi bi-icon-name me-2"></i>Page Title</h1>
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb mb-0">
                <li class="breadcrumb-item"><a href="parametres.html">Parametres</a></li>
                <li class="breadcrumb-item"><a href="parametres.html#section">Section</a></li>
...
document.addEventListener('DOMContentLoaded', async () => {
  // Initialize admin template (navbar + sidebar)
  if (typeof initTemplate === 'function') {
    await initTemplate('parametres'); // or 'catalogue', 'collections', etc.
  }
...
**IMPORTANT**: Never use CDN links for external JavaScript libraries. All external libraries must be downloaded to `frontend/vendor/`.

To add a new library:
1. Create folder: `frontend/vendor/library-name/`
2. Download minified JS/CSS files using curl or manually
3. Reference with absolute path: `/vendor/library-name/file.min.js`

Example:
```bash
# Download a library
mkdir -p frontend/vendor/newlib
curl -L -o frontend/vendor/newlib/newlib.min.js "https://cdn.jsdelivr.net/npm/newlib@version/dist/newlib.min.js"
```

Current vendor libraries:
- `bootstrap/` - Bootstrap 5.x CSS + JS bundle
- `bootstrap-icons/` - Icon font + CSS
- `sweetalert2/` - Modal dialogs and toast notifications
- `sortablejs/` - Drag & drop list sorting
- `quill/` - Rich text WYSIWYG editor
- `html5-qrcode/` - Camera-based QR/barcode scanner

**Note**: Some pages still use CDN for tom-select, chart.js, flatpickr, and quagga2. These should be migrated to vendor when updated.
